...
pub fn fork_base(base_key: &str, fork_key: &str, root: &Path) -> Result<()> {
...
mod tests {
    ...
    fn fork_unknown_base_returns_not_found() {
        ...
        let err = fork_base("nonexistent-base", "agent-1", root.path())
    ...
    fn fork_duplicate_key_returns_already_exists() {
        ...
        let err =
            fork_base("mybase", "agent-1", root.path()).expect_err("expected AlreadyExists error");
        let msg = err.to_string();
        assert!(
